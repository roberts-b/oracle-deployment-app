...
ipcMain.on(rpcNames.TEST_CONNECTION.reqName, (event, args) => {
    log.info('test_connection received ', args);

    validateConnection(args).then(function (validationResult) {
        log.info('validateConnection returned value: ', validationResult);
        event.sender.send(rpcNames.TEST_CONNECTION.respName, { status: constants.SUCCESS_MESSAGE, value: validationResult });
    }).catch(function (error) {
        log.error('tnsListener received from test_connection reply: ', error);
        event.sender.send(rpcNames.TEST_CONNECTION.respName, { status: constants.FAILURE_MESSAGE, value: error.message });
    });
